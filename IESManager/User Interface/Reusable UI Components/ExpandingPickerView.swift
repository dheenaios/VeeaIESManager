...
protocol ExpandingPickerViewDelegate: AnyObject {
    func didSelectRow(row: Int, inComponent component: Int)
}

class ExpandingPickerView: UIView {
    
    weak var delegate: ExpandingPickerViewDelegate?
    ...
    public var valueText: String = "" {
        didSet {
            ...
            // TODO: Try and select the item that is the same as that text
        }
    }
    ...
    func setup(openHeight :CGFloat, ibHeightConstraint :NSLayoutConstraint, titleText :String, options: [String]) {
        ...
        // Value
        valueLabel.text = ""
        valueLabel.trailingAnchor.constraint(equalTo: headerView.trailingAnchor, constant: -16.0).isActive = true
        ...
        // Label
        valueLabel.font = FontManager.regular(size: 17)
        valueLabel.font = FontManager.regular(size: 15)
...
extension ExpandingPickerView: UIPickerViewDelegate {
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        guard let options = pickerOptions else { return }
