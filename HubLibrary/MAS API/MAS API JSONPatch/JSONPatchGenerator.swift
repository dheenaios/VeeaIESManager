...
struct JSONPatchGenerator {
    ...
    fileprivate enum Operation {
        ...
        //case copy(from: JSONPointer, path: JSONPointer, value: JSONElement) // (VHM-1020)
    ...
    private mutating func add(path: JSONPointer, value: JSONElement) {
        /*
         See VHM 1020 for a full explanation of why the following is commented out.
         The COPY / MOVE operations SHOULD have the same effect as the add operation BUT the backend does not seem to like the COPY or MOVE command.
         So remove it and just use a simple add, as the effect will be the same
         */
...
//        if let oldPath = findUnchangedValue(value: value, path: path) {
...
//        }
    ...
    // THE ORIGINAL METHOD
//    private func findUnchangedValue(value: JSONElement, path: JSONPointer) -> JSONPointer? {
//        // NEED TO CHECK THE PATH AS WELL AS THE VALUE
//
...
//            self = .copy(from: from, path: path) // (VHM-1020)
